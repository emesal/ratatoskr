...
impl HfTokenizer {
    ...
    pub fn from_hub(repo_id: &str) -> Result<Self> {
        let api = hf_hub::api::sync::Api::new()
            .map_err(|e| RatatoskrError::Configuration(format!("Failed to initialize HF API: {}", e)))?;
    ...
    pub fn from_file(path: &Path) -> Result<Self> {
        let inner = tokenizers::Tokenizer::from_file(path).map_err(|e| {
            RatatoskrError::Configuration(format!("Failed to load tokenizer from {:?}: {}", path, e))
        })?;
...
impl TokenizerProvider for HfTokenizer {
    fn count_tokens(&self, text: &str) -> Result<usize> {
        let encoding = self.inner.encode(text, false).map_err(|e| {
            RatatoskrError::DataError(format!("Tokenization failed: {}", e))
        })?;
