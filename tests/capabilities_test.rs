...
fn test_capabilities_huggingface_only() {
    ...
    assert!(
        ...
        "huggingface_only should not have chat"
    );
    assert!(
    ...
    assert!(
        caps.has(ModelCapability::Embed),
        "huggingface_only should have embeddings"
    );
    assert!(
        caps.has(ModelCapability::Nli),
        "huggingface_only should have nli"
    );
    assert!(
        caps.has(ModelCapability::Classify),
...
fn test_capabilities_merge() {
    ...
    assert!(
        merged.has(ModelCapability::ChatStreaming),
        "merged should have chat_streaming"
    );
    assert!(
        merged.has(ModelCapability::Embed),
        "merged should have embeddings"
    );
    assert!(merged.has(ModelCapability::Nli), "merged should have nli");
    assert!(
        merged.has(ModelCapability::Classify),
        "merged should have classification"
    );
    assert!(
...
fn test_capabilities_local_only() {
    ...
    assert!(
        !caps.has(ModelCapability::Chat),
        "local_only should not have chat"
    );
    assert!(
        !caps.has(ModelCapability::ChatStreaming),
    ...
    assert!(
        !caps.has(ModelCapability::Generate),
        "local_only should not have generate"
    );
    assert!(
        !caps.has(ModelCapability::ToolUse),
        "local_only should not have tool_use"
    );
    assert!(
        caps.has(ModelCapability::Embed),
        "local_only should have embeddings"
    );
    ...
    assert!(
        caps.has(ModelCapability::TokenCounting),
        "local_only should have token_counting"
    );
    assert!(
...
fn test_capabilities_full() {
    ...
    assert!(caps.has(ModelCapability::Stance));
